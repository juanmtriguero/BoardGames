...
<display:table name="promotions" id="row" requestURI="/event/auth/byBusiness.do" pagesize="5" class="displaytag">
	...
	<display:column title="${chronicleHeader}" sortable="false">
		<jstl:if test="${row.chronicle==null}">
			<spring:message code="event.nochronicle" />
		</jstl:if>
		<jstl:if test="${row.chronicle!=null}">
	...
	<security:authorize access="hasRole('PLAYER')">
		...
		<display:column title="${inscriptionHeader}" sortable="false">
			<jstl:if test="${isClosed[row.id]==false}">
				<jstl:if test="${isJoined[row.id]==false}">
					<a href="event/player/join.do?eventId=${row.id}">
						<spring:message code="inscription.join" />
					</a>
				</jstl:if>
				<jstl:if test="${isJoined[row.id]==true}">
					<a href="event/player/unjoin.do?eventId=${row.id}">
						<spring:message code="inscription.unjoin" />
					</a>
				</jstl:if>
			</jstl:if>
			<jstl:if test="${isClosed[row.id]==true}">
...
<display:table name="tournaments" id="row" requestURI="/event/auth/byBusiness.do" pagesize="5" class="displaytag">
